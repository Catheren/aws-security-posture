...
GuardDuty, Security Hub CSPM, Config, and Inspector are all pay-as-you-go.
With three accounts in scope expect roughly 3x a single-account estimate.
For a personal sandbox this is typically a few dollars over the project
duration. Disable services and destroy resources when not actively working
to minimize ongoing charges — capture screenshots or export findings first,
since disabling detectors clears their history.
